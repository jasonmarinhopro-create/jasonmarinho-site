...
export default async function ChezNousPostPage({ params }: Props) {
  ...
  const { data: post } = await supabase
    ...
    .select('id, author_id, category, title, body, pinned, locked, reply_count, vote_count, created_at, edited_at')
    .eq('id', postId)
    .maybeSingle()
  ...
  const [{ data: usersData }, { data: myVote }, ...badgeQueries] = await Promise.all([
    userIds.length ? supabase.from('profiles').select('id, full_name, pseudo, role, is_contributor, created_at').in('id', userIds) : Promise.resolve({ data: [] }),
