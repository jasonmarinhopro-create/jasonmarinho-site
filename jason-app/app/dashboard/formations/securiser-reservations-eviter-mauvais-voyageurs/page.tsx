...
export default async function FormationPage() {
  ...
  const { data: formation } = await supabase
    .from('formations')
    .select('id')
    .eq('slug', 'securiser-reservations-eviter-mauvais-voyageurs')
    .single()
  ...
  const formationContent = await getFormationDbContent(formationId, SECURISER_RESERVATIONS_FORMATION)

  return (
    <>
      <Header title="Formation Sécuriser ses réservations" userName={profile?.full_name ?? undefined} />
      <FormationView
