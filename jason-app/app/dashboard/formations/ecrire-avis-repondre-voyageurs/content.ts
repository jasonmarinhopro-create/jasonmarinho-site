export const ECRIRE_AVIS_FORMATION = {
  slug: 'ecrire-avis-repondre-voyageurs',
  title: 'Écrire des avis et répondre aux voyageurs',
  description: 'Les avis sont le moteur n°1 de vos réservations. Apprenez à les collecter, à rédiger les vôtres, à répondre aux négatifs sans jamais perdre de clients, et à transformer chaque commentaire en levier de croissance.',
  duration: '2h00',
  level: 'Débutant',
  objectifs: [
    'Comprendre comment les avis impactent le classement Airbnb et Booking',
    'Savoir rédiger des avis voyageurs honnêtes et utiles',
    `Maîtriser la structure d'une réponse à un avis négatif`,
    `Collecter plus d'avis positifs sans violer les règles des plateformes`,
    'Transformer les critiques en améliorations concrètes',
    'Gérer les avis injustes ou de mauvaise foi',
  ],
  modules: [
    {
      id: 1,
      title: 'Pourquoi les avis sont votre actif le plus précieux',
      duration: '25 min',
      lessons: [
        {
          id: 1,
          title: `L'impact direct des avis sur vos réservations`,
          duration: '12 min',
          content: `---

MODULE 1 · LEÇON 1
────────────────────────────────────
L'impact des avis
Votre actif le plus précieux en LCD
────────────────────────────────────

---

## Les chiffres qui montrent tout
Les avis influencent directement
vos revenus, dans les deux sens.

→ Une note de 4,8+ sur Airbnb permet
  de pratiquer des tarifs 12 à 18%
  au-dessus de la moyenne locale.
  Source : AirDNA, 2026

→ Les plateformes valorisent les logements
  avec un bon volume d'avis récents :
  plus d'avis = meilleur classement =
  plus de visibilité = plus de réservations.

→ Un seul avis négatif bien géré
  n'entache pas votre réputation.
  Mais un avis négatif sans réponse
  fait fuir les futurs voyageurs.

Les avis ne sont pas optionnels.
Ce sont les fondations de votre activité.

---

## Comment les plateformes utilisent vos avis
AIRBNB
→ Prend en compte la note globale,
  la note sur 6 critères spécifiques
  (propreté, précision, communication,
   emplacement, arrivée, rapport qualité/prix),
  la récence des avis et leur volume.
→ Le badge Superhôte nécessite
  une note ≥ 4,8 et un taux de réponse ≥ 90%.

BOOKING.COM
→ Calcule une note sur 10 basée
  sur des critères pondérés.
→ Les avis récents comptent plus
  que les anciens.
→ La réactivité de l'hôte est prise
  en compte dans le classement.

GOOGLE MY BUSINESS
→ Note sur 5, visible directement
  dans les résultats Google.
→ Les réponses de l'hôte sont
  indexées par Google.

→ Dans tous les cas : répondre,
  c'est aussi parler aux algorithmes.

---

## Les 6 critères Airbnb à surveiller
Airbnb note les hôtes sur 6 dimensions :

01. PROPRETÉ
    La plus scrutée. Un 4 sur ce critère
    peut faire basculer une réservation.

02. PRÉCISION DE L'ANNONCE
    Le logement correspond-il aux photos
    et à la description ?

03. COMMUNICATION
    Rapidité et qualité des réponses
    avant et pendant le séjour.

04. EMPLACEMENT
    Difficile à améliorer, mais la
    description doit être honnête.

05. ARRIVÉE
    Le check-in s'est-il passé
    sans friction ?

06. RAPPORT QUALITÉ / PRIX
    Le voyageur a-t-il le sentiment
    d'avoir eu ce qu'il a payé ?

→ Identifier votre critère le plus faible
  et l'améliorer est souvent plus efficace
  que de tout optimiser en même temps.

---

## La règle des 14 jours
Sur Airbnb :
→ Hôte et voyageur ont 14 jours
  après le check-out pour laisser un avis.
→ Les deux avis sont publiés simultanément
  quand les deux ont répondu,
  ou à la fin des 14 jours si un seul répond.
→ Passé ce délai : impossible de laisser
  ou modifier un avis.

Sur Booking :
→ Booking envoie automatiquement une
  invitation à noter après le séjour.
→ Les hôtes peuvent répondre à tout moment
  après la publication de l'avis voyageur.

Sur Google My Business :
→ Pas de délai, les avis peuvent
  être laissés à tout moment.
→ Tu peux répondre à tout moment.

→ Implication pratique :
  Votre demande d'avis doit être
  envoyée dans les 24-48h après le départ.
  Pas une semaine après.

---


## Exercice
**Exercice pratique :** Regarde maintenant les 6 critères de ta dernière période de réservations sur Airbnb. Quel est ton critère le plus faible ? C'est là que tu dois concentrer tes efforts en premier, pas sur ce qui va déjà bien.

---`,
        },
        {
          id: 2,
          title: `Collecter plus d'avis sans violer les règles`,
          duration: '13 min',
          content: `---

MODULE 1 · LEÇON 2
────────────────────────────────────
Collecter plus d'avis
Les méthodes qui fonctionnent, et celles qui sont interdites
────────────────────────────────────

---

## Ce qui est strictement interdit
Les plateformes sont très claires
sur ce qui est INTERDIT :

❌ Promettre un remboursement,
   une réduction ou un avantage
   en échange d'un bon avis.

❌ Demander à un ami ou un proche
   de laisser un faux avis.

❌ Supprimer ou contester un avis
   simplement parce qu'il est négatif
   (sans raison valable).

❌ Harceler un voyageur avec des relances
   répétées pour qu'il laisse un avis.

❌ Conditionner votre propre avis
   sur le voyageur à sa note sur vous.

Sanctions possibles : suppression
de l'annonce, suspension du compte,
retrait de l'avis et de votre réponse.

---

## Les méthodes légitimes et efficaces
MÉTHODE 1, La demande dans le message de check-out
Le moment le plus naturel.
"Si vous avez passé un bon moment,
un avis nous ferait vraiment plaisir."
→ Simple, dans le contexte du départ,
  sans pression.

MÉTHODE 2, Le QR code dans le logement
Un cadre encadré, posé sur la table :
"Vous avez aimé votre séjour ?
Laissez-nous un avis Google 👇"
+ QR code vers votre fiche GMB.
→ Discret, efficace, disponible 24h/24.

MÉTHODE 3, Laisser votre avis en premier
Sur Airbnb, dès que vous laissez
votre avis sur le voyageur,
il reçoit une notification.
→ La réciprocité joue : 
  ils pensent à laisser le leur.

MÉTHODE 4, Le message J+1
Un message le lendemain du départ,
chaleureux, qui glisse la demande
naturellement sans en faire l'objet principal.

---

## Le timing parfait pour demander un avis
La fenêtre idéale :
→ Entre 12h et 48h après le départ.

Trop tôt (pendant le séjour) :
→ Le voyageur n'a pas tout vécu.
→ Impression de pression.

Trop tard (après 3-4 jours) :
→ L'enthousiasme du séjour est retombé.
→ Le voyageur est passé à autre chose.
→ Fenêtre de 14 jours qui se referme.

Le jour J+1 est généralement
le meilleur moment :
→ Le voyageur est rentré chez lui.
→ Il repense à son séjour avec recul.
→ Si l'expérience était bonne,
  il est dans les meilleures dispositions
  pour laisser un avis enthousiaste.

---

## Augmenter la qualité des avis reçus
Un avis "super séjour !" avec 5 étoiles
vaut beaucoup moins qu'un avis détaillé
qui mentionne la propreté, la localisation
et la communication.

Pourquoi ? Parce que Google et les futures
plateformes "lisent" les avis.
Les mots utilisés renforcent votre
référencement sur certaines recherches.

Pour obtenir des avis détaillés :
→ Posez une question dans votre demande :
  "Si vous deviez conseiller notre logement
  à un ami, qu'est-ce que vous lui diriez ?"
→ Cette formulation incite naturellement
  à développer plutôt que juste noter.

---


## Exercice
**Exercice pratique :** Vérifie que tu laisses bien un avis sur chaque voyageur dans les 48h après le départ. Si ce n'est pas fait systématiquement, crée un rappel dans ton téléphone déclenchant à J+1 après chaque check-out. C'est la première action pour augmenter ton volume d'avis reçus via la réciprocité.

---`,
        },
      ],
    },
    {
      id: 2,
      title: 'Rédiger des avis voyageurs utiles et honnêtes',
      duration: '25 min',
      lessons: [
        {
          id: 11,
          title: `La structure d'un bon avis positif`,
          duration: '12 min',
          content: `---

MODULE 2 · LEÇON 1
────────────────────────────────────
Rédiger un avis positif
Utile pour les futurs hôtes, rapide pour vous
────────────────────────────────────

---

## Pourquoi vos avis sur les voyageurs comptent
Quand vous laissez un avis sur un voyageur,
vous contribuez à la communauté.

Les futurs hôtes lisent ces avis
pour décider d'accepter ou refuser
une demande de réservation.

Un bon avis voyageur :
→ Aide les autres hôtes à prendre
  de bonnes décisions
→ Récompense un bon comportement
→ Encourage le voyageur à soigner
  ses futurs séjours

Un avis vague ou générique
n'aide personne.

"Excellent voyageur, séjour parfait !"
→ Inutile.

"[Prénom] a communiqué rapidement,
respecté le logement et rendu les lieux
propres à son départ. Aucune question
pendant le séjour, on recommande !"
→ Utile.

---

## La structure d'un avis positif efficace en 3 parties
PARTIE 1, L'identité du séjour (1 phrase)
Contexte qui personnalise l'avis :
"[Prénom] est venu(e) pour [raison
implicite, weekend, déplacement pro...]"
ou juste "[Prénom] a séjourné [X nuits]
dans notre logement."

PARTIE 2, Les points forts concrets (2-3 phrases)
Pas de généralités : des faits observables.
→ Communication : réactivité, clarté
→ Comportement : respect du règlement,
  soin du logement
→ Propreté à la remise des clés
→ Respect des horaires
→ Discrétion, absence de nuisance

PARTIE 3, La recommandation (1 phrase)
"Nous recommandons [Prénom] sans hésitation
à tous les hôtes."
ou
"Ce serait un plaisir de l'accueillir
à nouveau."

---

## Exemples concrets
✅ BON AVIS POSITIF

"Marie est une voyageuse exemplaire.
Communication fluide avant l'arrivée,
logement rendu dans un état impeccable,
respect total du règlement intérieur.
Un séjour sans friction du début à la fin.
On l'accueillerait sans hésitation."

✅ BON AVIS POSITIF (court)

"Thomas a été discret, propre et ponctuel.
RAS de notre côté, on recommande."

❌ MAUVAIS AVIS (trop vague)

"Super voyageur, tout s'est très bien passé !"
→ N'informe pas les autres hôtes.
  Impossible de savoir ce qui s'est bien passé.

---

## La recommandation "public" vs note privée
Sur Airbnb, vous avez deux options :

1. L'AVIS PUBLIC
   Visible par tous sur votre profil
   et celui du voyageur.
   → Soyez honnête mais professionnel.
   → Pas de détails inutilement blessants.

2. LES COMMENTAIRES PRIVÉS
   Visibles uniquement par le voyageur.
   → Idéal pour des retours constructifs
     que vous ne souhaitez pas rendre publics.
   → Ex : "La vaisselle n'avait pas été faite
     mais tout le reste était parfait."

Règle générale :
→ Les points positifs = commentaire public.
→ Les petits reproches sans gravité
  = commentaire privé ou omission.
→ Les manquements sérieux
  = mentionnés publiquement,
  avec des faits et sans agressivité.

---


## Exercice
**Exercice pratique :** Pense à ton dernier bon voyageur. Rédige son avis maintenant en 3 phrases en suivant la structure de la slide 3. Si le délai de 14 jours n'est pas dépassé, publie-le. Sinon, conserve ce template, tu t'en serviras pour le prochain.

---`,
        },
        {
          id: 12,
          title: 'Rédiger un avis négatif sur un voyageur sans se mettre à dos la plateforme',
          duration: '13 min',
          content: `---

MODULE 2 · LEÇON 2
────────────────────────────────────
Rédiger un avis négatif sur un voyageur
Honnête, factuel, professionnel
────────────────────────────────────

---

## Quand laisser un avis négatif
Laisser un avis négatif est difficile.
Mais ne pas le faire nuit à la communauté.

Tu dois laisser un avis négatif si :

→ Le voyageur a causé des dégâts
→ Le règlement intérieur a été
  violé de façon significative
  (fête, fumeurs, animaux non autorisés...)
→ Le nombre d'occupants non déclarés
→ Le logement a été rendu dans
  un état dégradé
→ La communication était problématique
  ou menaçante

Tu peux omettre ou minimiser si :
→ C'est un oubli mineur (vaisselle non faite)
→ Le voyageur a prévenu et s'est excusé
→ Le séjour était globalement correct
  malgré un petit accroc

---

## La règle d'or : factuel, jamais émotionnel
La tentation :
"Voyageur irrespectueux, logement
laissé dans un état scandaleux,
à éviter absolument !"

Le problème :
→ Subjectif = contestable
→ Émotionnel = peu crédible
→ Agressif = vous fait passer pour
  l'hôte difficile

Ce qui fonctionne :
→ Des faits observables et vérifiables
→ Un ton neutre, professionnel
→ Pas d'attaque personnelle

Exemple :
"Le logement a été rendu avec des dégâts
sur [objet] et un niveau de propreté
en dessous de nos standards habituels.
La communication pendant le séjour
a été limitée. Nous ne recommandons
pas ce voyageur."

---

## Structure d'un avis négatif professionnel
PARTIE 1, Constat factuel (1-2 phrases)
Décrivez ce qui s'est passé,
sans interprétation ni jugement de valeur.
→ "Le logement a été rendu avec [constat précis]."
→ "Le règlement intérieur n'a pas été respecté
  concernant [point précis]."

PARTIE 2, Impact si pertinent (1 phrase, optionnel)
"Cela a nécessité [réparation / nettoyage
supplémentaire / intervention d'urgence]."

PARTIE 3, Conclusion (1 phrase)
Directe et sans ambiguïté :
"Nous ne recommandons pas ce voyageur."
ou
"Nous conseillons aux hôtes de demander
un dépôt de garantie avant d'accepter."

→ Jamais plus de 5-6 lignes.
  Un avis long semble une vendetta.
  Un avis court et factuel semble crédible.

---

## Avant de publier : la checklist
Avant de publier un avis négatif,
pose-toi ces 4 questions :

□ Est-ce que j'ai attendu au moins
  24h après la colère initiale ?
  → Si non : attends encore.

□ Mes affirmations sont-elles toutes
  prouvables (photos, échanges écrits) ?
  → Si non : retire les points non prouvables.

□ Est-ce que je règle un problème
  personnel ou j'informe la communauté ?
  → Si c'est personnel : note privée suffit.

□ Ma réponse serait-elle acceptable
  si c'était moi le voyageur ?
  → Si non : reformule.

Et souviens-toi : ta réponse est
lue par tes futurs voyageurs.
Un hôte qui semble "difficile à vivre"
perd des réservations.

---


## Exercice
**Exercice pratique :** Si tu as un voyageur difficile dont tu n'as pas encore laissé l'avis (et que le délai de 14 jours n'est pas dépassé) : rédige-le maintenant avec la structure de la slide 4. Factuel, court, professionnel. Pas plus de 5 lignes.

---`,
        },
      ],
    },
    {
      id: 3,
      title: `Répondre aux avis négatifs : l'art de transformer une critique en atout`,
      duration: '35 min',
      lessons: [
        {
          id: 21,
          title: 'La psychologie derrière un avis négatif',
          duration: '11 min',
          content: `---

MODULE 3 · LEÇON 1
────────────────────────────────────
La psychologie de l'avis négatif
Comprendre avant de répondre
────────────────────────────────────

---

## Ce que les futurs voyageurs cherchent vraiment
Quand un futur voyageur lit un avis négatif,
il ne cherche pas à savoir si votre logement
est parfait.

Il cherche à savoir si VOUS êtes
un hôte de confiance.

Sa vraie question :
"Si quelque chose se passe mal pendant
mon séjour, comment cet hôte va-t-il
réagir ?"

Votre réponse à un avis négatif
est la démonstration directe
de votre façon de gérer les problèmes.

→ Une réponse calme, professionnelle
  et constructive rassure bien plus
  qu'une annonce sans avis négatif.

→ Une réponse défensive ou agressive
  confirme les pires craintes du lecteur.

---

## Les 3 types d'avis négatifs
TYPE 1, L'AVIS LÉGITIME
Le voyageur a vécu quelque chose
de réel et le décrit honnêtement.
→ Reconnaître, s'améliorer, le montrer.

TYPE 2, L'AVIS EXCESSIF
Le problème était réel mais mineur.
Le voyageur a sur-réagi ou est
intrinsèquement difficile à satisfaire.
→ Répondre factuellement en remettant
  les choses dans leur contexte.
  Sans agressivité.

TYPE 3, L'AVIS INJUSTE OU DE MAUVAISE FOI
Informations fausses, harcèlement,
voyageur qui cherche un remboursement.
→ Répondre factuellement + signaler
  à la plateforme si nécessaire.

Erreur fréquente : traiter tous les
avis négatifs comme de type 3.
La plupart sont de type 1 ou 2.

---

## Ce qu'il ne faut JAMAIS faire
❌ Répondre à chaud (dans l'heure
   ou le lendemain si tu es encore énervé)

❌ Attaquer le voyageur personnellement
   ("ce voyageur est malhonnête / menteur")

❌ Nier des faits avérés
   ("ce n'est absolument pas vrai")
   → Si c'est partiellement vrai :
     reconnais la partie vraie.

❌ Lister tous les reproches en réponse
   point par point → ressemble à une plaidoirie

❌ Promettre des choses vagues
   ("nous allons tout améliorer")
   sans détail concret

❌ Copier-coller la même réponse
   sur tous vos avis négatifs
   → Les futurs voyageurs le voient.

---

## Le délai idéal pour répondre
Sur Airbnb :
→ Tu as 14 jours à compter de la
  publication de l'avis pour répondre.

Recommandation :
→ Prends 24 à 48h avant de répondre
  (pour éviter la réponse à chaud).
→ Ne dépasse pas 5-7 jours.
→ Un avis sans réponse après une semaine
  semble ignoré, message négatif
  pour les futurs voyageurs.

Sur Booking :
→ Pas de délai strict pour répondre.
→ Même recommandation : 24-48h.

Sur Google My Business :
→ Pas de délai.
→ Idéalement sous 48-72h.
→ Google favorise les profils actifs
  dans son algorithme.

---


## Exercice
**Exercice pratique :** Pense au dernier avis négatif que tu as reçu. Dans quel type tombe-t-il : légitime, excessif, ou injuste ? Cette classification change complètement la façon dont tu dois répondre. On voit ça dans la leçon suivante.

---`,
        },
        {
          id: 22,
          title: `La structure d'une réponse parfaite à un avis négatif`,
          duration: '13 min',
          content: `---

MODULE 3 · LEÇON 2
────────────────────────────────────
La structure d'une réponse parfaite
4 étapes qui transforment la critique en atout
────────────────────────────────────

---

## La structure en 4 étapes
ÉTAPE 1, REMERCIER (1 phrase)
Toujours. Même si ça fait mal.
"Merci pour votre retour."
→ Montre que vous prenez les avis
  au sérieux, pas seulement les positifs.

ÉTAPE 2, RECONNAÎTRE (1-2 phrases)
Reconnaître le ressenti,
même si vous n'êtes pas entièrement d'accord.
"Nous comprenons que [X] n'a pas
correspondu à vos attentes."
→ Ne pas confondre reconnaître et admettre
  une faute que vous n'avez pas commise.

ÉTAPE 3, AGIR OU EXPLIQUER (1-2 phrases)
Ce que vous avez fait ou allez faire.
"Depuis ce séjour, nous avons [action concrète]."
ou
"Pour information, [élément de contexte
factuel qui éclaire la situation]."

ÉTAPE 4, CONCLURE POSITIVEMENT (1 phrase)
"Nous espérons pouvoir vous accueillir
à nouveau dans de meilleures conditions."
ou simplement :
"Nous prenons note pour améliorer
l'expérience de nos futurs voyageurs."

---

## Exemple : avis légitime
AVIS REÇU :
"La salle de bain était décevante -
 joint de douche à refaire, robinet
 qui gouttait. Dommage car le reste
 était très bien."

MAUVAISE RÉPONSE :
"Nous sommes désolés mais nos équipements
sont toujours vérifiés entre chaque séjour."

BONNE RÉPONSE :
"Merci pour votre retour honnête.
Vous avez tout à fait raison concernant
la salle de bain, ces défauts avaient
été signalés mais pas encore traités.
Les réparations ont été effectuées
dans la semaine suivant votre séjour.
Nous espérons vous revoir pour un séjour
qui corresponde davantage à vos attentes."

→ Pourquoi ça fonctionne :
  Honnête, factuel, action concrète.
  Le futur voyageur voit que vous agissez.

---

## Exemple : avis excessif
AVIS REÇU :
"Logement correct mais le quartier
est trop bruyant le soir. Je ne savais
pas qu'il y avait autant de bars.
Nuits difficiles. Je déconseille."

MAUVAISE RÉPONSE :
"Le quartier est clairement décrit
dans notre annonce, nous ne pouvons
pas être tenus responsables du bruit."

BONNE RÉPONSE :
"Merci pour votre retour.
Nous sommes sincèrement navrés
que les nuisances sonores du quartier
aient perturbé vos nuits.
L'emplacement de notre logement,
au cœur du quartier festif, est mentionné
dans notre annonce, nous y ajouterons
une mention plus explicite sur l'animation
nocturne pour mieux informer
les futurs voyageurs.
Bonne continuation."

→ Pourquoi ça fonctionne :
  On reconnaît le ressenti, on améliore
  l'information sans accepter une faute.
  On parle AUX FUTURS VOYAGEURS.

---

## Exemple : avis de mauvaise foi
AVIS REÇU :
"Hôte qui ment sur ses équipements.
Lave-vaisselle en panne, chauffage
insuffisant. Évitez."

(Le lave-vaisselle fonctionnait.
Le chauffage était à 19°C.)

MAUVAISE RÉPONSE :
"Ce voyageur ment. Le lave-vaisselle
fonctionnait parfaitement et nous
avons les photos pour le prouver."

BONNE RÉPONSE :
"Merci pour votre retour.
Le lave-vaisselle était en bon état
lors de votre séjour, aucun incident
ne nous a été signalé pendant ou après.
Le thermostat était réglé à 19°C
conformément à nos paramètres habituels ;
n'hésitez pas à nous contacter
lors de votre prochain séjour
pour ajuster selon vos préférences.
Nous sommes à la disposition
de la plateforme pour tout justificatif."

→ Factuel, calme, ouvert.
  Les futurs voyageurs font la différence.

---


## Exercice
**Exercice pratique :** Prends le dernier avis négatif reçu et rédige une réponse en suivant les 4 étapes. Remercier → Reconnaître → Agir/Expliquer → Conclure. Maximum 6 lignes. Soumets-la mentalement au test : "Est-ce que ce message rassure un futur voyageur qui hésite à réserver ?" Si oui : publie-la.

---`,
        },
        {
          id: 23,
          title: 'Contester un avis injuste : quand et comment',
          duration: '11 min',
          content: `---

MODULE 3 · LEÇON 3
────────────────────────────────────
Contester un avis injuste
Quand c'est possible, et comment s'y prendre
────────────────────────────────────

---

## Ce que les plateformes acceptent de supprimer
NI AIRBNB NI BOOKING ne suppriment
un avis simplement parce qu'il est négatif.

Ce qu'Airbnb peut supprimer :

✓ Avis contenant des informations
  fausses et vérifiables
✓ Avis discriminatoire (racisme,
  sexisme, homophobie...)
✓ Avis publié en représailles
  (le voyageur l'avoue dans les échanges)
✓ Avis d'un voyageur dont le séjour
  a été annulé (jamais réalisé)
✓ Avis contenant des informations
  personnelles (adresse, numéro...)
✓ Avis violant les CGU d'Airbnb

Ce qu'Airbnb NE supprime PAS :
→ Un avis négatif honnête même sévère
→ Un avis avec lequel vous êtes en désaccord
→ Un avis qui vous dérange
  mais qui respecte les règles

---

## Comment signaler un avis abusif sur Airbnb
ÉTAPE 1
Rassemble tes preuves en amont :
→ Captures d'écran des échanges de messages
→ Photos horodatées de l'état du logement
→ Témoignage d'un prestataire si applicable

ÉTAPE 2
Va dans le fil de conversation
avec le voyageur sur Airbnb
→ Option "Signaler un problème"

OU

Accède directement à l'avis
sur ton tableau de bord hôte
→ "Contester cet avis"

ÉTAPE 3
Décris factuellement pourquoi l'avis
viole les CGU d'Airbnb.
Joint tes preuves.

ÉTAPE 4
Airbnb étudie la demande.
Délai habituel : 3 à 10 jours ouvrés.
Le résultat n'est pas garanti.

---

## La stratégie quand la contestation échoue
Si Airbnb refuse de supprimer l'avis
(ce qui arrive dans la majorité des cas) :

→ Ta meilleure arme reste ta réponse publique.
  Une réponse factuelle, calme et documentée
  fait plus de bien qu'un avis supprimé.

→ Les futurs voyageurs voient les deux.
  Ils sont capables de juger.

→ Continue à collecter des avis positifs.
  Un avis négatif isolé parmi 30 positifs
  ne change pas grand chose.
  Un avis négatif isolé parmi 5 avis total :
  c'est là le vrai problème.

→ La meilleure réponse à un mauvais avis
  reste un volume plus élevé de bons avis.

---


## Exercice
**Exercice pratique :** As-tu un avis qui te semble vraiment injuste ou abusif ? Vérifie s'il correspond à l'un des critères de suppression d'Airbnb listés en slide 2. Si oui : prépare tes preuves et lance la contestation. Si non : concentre ton énergie sur la réponse publique et la collecte de nouveaux avis positifs. C'est plus efficace.

---`,
        },
      ],
    },
    {
      id: 4,
      title: 'Transformer les avis en amélioration continue',
      duration: '35 min',
      lessons: [
        {
          id: 31,
          title: 'Lire entre les lignes : ce que vos avis révèlent vraiment',
          duration: '12 min',
          content: `---

MODULE 4 · LEÇON 1
────────────────────────────────────
Lire entre les lignes
Ce que vos avis révèlent sur votre logement
────────────────────────────────────

---

## Les mots qui reviennent = ce qui compte vraiment
Vos avis sont une mine d'informations.
La plupart des hôtes les lisent une fois
et passent à autre chose.
Les meilleurs hôtes les analysent.

Méthode simple :
→ Prenez vos 20 derniers avis.
→ Notez les mots qui reviennent
  dans les avis positifs.
→ Notez les mots qui reviennent
  dans les critiques.

Exemples courants :

Positifs fréquents :
"propre", "pratique", "calme",
"bien équipé", "hôte réactif"
→ Ce sont vos atouts à mettre en avant
  dans votre annonce.

Critiques fréquentes :
"wifi lent", "matelas inconfortable",
"cuisine mal équipée", "bruit"
→ Ce sont vos priorités d'amélioration.

---

## Les critiques implicites à ne pas rater
Certaines critiques ne disent pas
directement ce qui ne va pas.

DÉCODAGE :

"Le logement correspond à la description"
→ Attention : c'est souvent un avis mitigé.
   Le voyageur dit que c'était correct
   mais pas excellent.

"Hôte disponible et réactif"
→ Il a fallu contacter l'hôte.
   Quelque chose n'était pas clair
   ou ne fonctionnait pas.

"Bien pour le prix"
→ Le rapport qualité/prix est le seul
  point positif notable.

"Pratique pour une nuit"
→ Le logement n'est pas assez agréable
  pour un séjour plus long.

Ces phrases sonnent positives
mais cachent des attentes non comblées.

---

## Créer sa liste d'améliorations prioritaires
Après avoir analysé vos avis,
classez les améliorations en 3 niveaux :

NIVEAU 1, QUICK WINS (< 50€, 1 journée)
→ Remplacer un équipement défaillant
→ Acheter des protège-oreilles
→ Améliorer la signalétique
→ Ajouter une lampe dans un coin sombre

NIVEAU 2, AMÉLIORATIONS MOYEN TERME (50-500€)
→ Changer un matelas
→ Acheter une meilleure cafetière
→ Améliorer la connexion Wi-Fi
→ Ajouter un équipement manquant

NIVEAU 3, INVESTISSEMENTS LOURDS (> 500€)
→ Rénovation partielle
→ Isolation phonique
→ Changement de mobilier majeur

→ Commencez toujours par le niveau 1.
  L'impact sur les avis est souvent
  disproportionné par rapport au coût.

---


## Exercice
**Exercice pratique :** Prends tes 10 derniers avis. Note sur une feuille les 3 mots qui reviennent le plus en positif et les 3 qui reviennent en critique. Identifie une action concrète de niveau 1 (moins de 50€) que tu peux faire cette semaine pour répondre à la critique la plus fréquente.

---`,
        },
        {
          id: 32,
          title: 'Répondre aux avis positifs : oui, ça compte aussi',
          duration: '11 min',
          content: `---

MODULE 4 · LEÇON 2
────────────────────────────────────
Répondre aux avis positifs
L'étape que 90% des hôtes ignorent
────────────────────────────────────

---

## Pourquoi répondre aux avis positifs
La plupart des hôtes ne répondent
qu'aux avis négatifs.

C'est une erreur pour 3 raisons :

01. SIGNAL ALGORITHME
    Google et Booking valorisent les profils
    actifs dans leur algorithme.
    Répondre aux avis = signal d'activité.

02. EXPÉRIENCE VOYAGEUR
    Un voyageur qui voit sa réponse
    se souvient de vous.
    Fidélisation et bouche-à-oreille.

03. MARKETING INDIRECT
    Votre réponse s'affiche publiquement.
    C'est de l'espace pour parler
    à vos futurs voyageurs.
    Utilisez-le intelligemment.

---

## La structure d'une réponse à un avis positif
MOINS de 3 lignes. Toujours.

STRUCTURE :
1. Remerciement personnalisé
   (mentionner un détail de l'avis)
2. Une phrase sur votre engagement
   ou ce que vous aimez dans votre métier
3. Invitation à revenir (optionnel)

EXEMPLES :

"Merci beaucoup [Prénom] !
Votre mention sur la propreté
nous touche particulièrement -
c'est notre priorité absolue.
Au plaisir de vous revoir !"

"Merci pour ce retour qui nous fait
vraiment plaisir. Accueillir des voyageurs
comme vous, c'est ce qui rend ce métier
si enrichissant. À bientôt !"

❌ À ÉVITER :
"Merci beaucoup ! Nous sommes ravis
que votre séjour se soit bien passé.
N'hésitez pas à revenir !"
→ Trop générique, copié-collé,
  impersonnel. Contre-productif.

---

## La règle du 80/20 sur les réponses
Vous n'avez pas le temps de répondre
à 100% de vos avis positifs ?

Priorisez selon cette logique :

RÉPONDEZ TOUJOURS à :
→ Tous les avis négatifs (sans exception)
→ Les avis très détaillés et enthousiastes
→ Les avis de voyageurs qui reviennent

RÉPONDEZ QUAND VOUS POUVEZ à :
→ Les avis positifs courts mais sincères

OPTIONNEL :
→ Les avis très génériques
  ("Super séjour, je recommande")

→ L'objectif n'est pas la quantité
  de réponses mais leur qualité.
  Une réponse personnalisée vaut
  mieux que 10 réponses copiées-collées.

---


## Exercice
**Exercice pratique :** Identifie les 3 derniers avis positifs détaillés que tu as reçus. Rédige une réponse personnalisée pour chacun en moins de 3 lignes. Mentionne un élément spécifique de leur avis. Publie ces réponses aujourd'hui.

---`,
        },
        {
          id: 33,
          title: 'Construire une stratégie avis sur le long terme',
          duration: '12 min',
          content: `---

MODULE 4 · LEÇON 3
────────────────────────────────────
La stratégie avis long terme
Du réactif au proactif
────────────────────────────────────

---

## Le système complet en place
Une stratégie avis efficace s'automatise.

AVANT LE SÉJOUR
→ Communication claire et proactive
  = moins de déceptions à l'arrivée
  = moins d'avis négatifs

PENDANT LE SÉJOUR
→ Message de mi-séjour (si long séjour)
→ Disponibilité réelle pour les incidents
  = problèmes résolus avant le départ
  = pas transformés en avis négatifs

APRÈS LE SÉJOUR (J+1)
→ Message de remerciement + demande d'avis
→ Ton avis sur le voyageur (réciprocité)

CHAQUE MOIS
→ Analyse des mots-clés récurrents
→ 1 action d'amélioration identifiée
→ Réponses publiées sur les avis non traités

---

## L'objectif chiffré : la note 4,8
Sur Airbnb, 4,8 est le seuil
qui change tout :
→ Éligibilité au badge Superhôte
→ Visibilité accrue dans les résultats
→ Justification de tarifs plus élevés

Comment atteindre et maintenir 4,8 :

→ Propreté irréprochable systématique
  (c'est le critère le plus noté)
→ Communication réactive
  (répondre en moins de 1h)
→ Annonce précise et honnête
  (pas de surprise à l'arrivée)
→ Check-in sans friction
  (instructions claires la veille)
→ Petite attention à l'arrivée
  (café, chocolat, message personnalisé)

Ces 5 actions seules peuvent
faire passer une note de 4,5 à 4,8
en 3 mois.

---

## La note sur Google My Business : ne pas l'oublier
Beaucoup d'hôtes se concentrent
exclusivement sur Airbnb.

La note Google est pourtant visible
par n'importe qui sur n'importe quel
moteur de recherche, sans réservation.

Un futur propriétaire qui cherche
une conciergerie, un voyageur qui
hésite entre deux logements,
un journaliste local -
tous passent par Google.

Pour développer ta note Google :
→ Demande aux voyageurs satisfaits
  un avis Google via un lien direct
  ou un QR code dans le logement
→ Réponds à chaque avis Google
  (l'algorithme Google le valorise)
→ Mentionne ton lien GMB dans
  tes messages de fin de séjour
  pour les réservations directes

---

## Le récapitulatif de la formation
CE QUE TU SAIS MAINTENANT FAIRE :

MODULE 1
□ Comprendre l'impact des avis
  sur ton classement et tes revenus
□ Collecter plus d'avis sans violer
  les règles des plateformes

MODULE 2
□ Rédiger des avis voyageurs utiles
  (positifs ET négatifs)

MODULE 3
□ Répondre à un avis négatif en 4 étapes
□ Contester un avis injuste
  quand c'est justifié

MODULE 4
□ Analyser tes avis pour identifier
  tes points d'amélioration
□ Répondre aux avis positifs efficacement
□ Construire une stratégie avis
  qui tourne seule

→ Les avis ne sont pas une fatalité.
  Ce sont un levier que tu contrôles
  bien plus que tu ne le crois.

Des questions sur un avis spécifique ?
[Prends un appel avec Jason](/contact)

---


## Exercice
**L'action finale de la formation :**

Mets en place ces 3 automatismes cette semaine :

1. **Rappel J+1** dans ton téléphone après chaque check-out → envoyer le message de demande d'avis et laisser ton propre avis sur le voyageur.

2. **QR code GMB** imprimé et encadré dans le logement → visible au check-out.

3. **Analyse mensuelle** : agenda bloqué 30 minutes le premier lundi de chaque mois → relire les avis du mois, noter les mots récurrents, identifier 1 action d'amélioration.

Ces 3 habitudes suffisent pour construire une réputation solide en 6 mois.

Formatio nsuviante :`,
        },
      ],
    },
  ],
}
