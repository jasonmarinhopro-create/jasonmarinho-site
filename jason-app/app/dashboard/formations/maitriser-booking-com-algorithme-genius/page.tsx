...
export default async function BookingFormationPage() {
  ...
  const { data: formation } = await supabase
    .from('formations')
    .select('id')
    .eq('slug', 'maitriser-booking-com-algorithme-genius')
    .single()
  ...
  const formationContent = await getFormationDbContent(formationId, BOOKING_FORMATION)

  return (
    <>
      <Header title="Formation Booking.com" userName={profile?.full_name ?? undefined} />
      <FormationView
