...
export default async function FormationPage() {
  ...
  const { data: formation } = await supabase
    .from('formations')
    .select('id')
    .eq('slug', 'fiscalite-statut-conciergerie-tourisme')
    .single()
  ...
  const formationContent = await getFormationDbContent(formationId, FISCALITE_CONCIERGERIE_FORMATION)

  return (
    <>
      <Header title="Formation Fiscalité conciergerie de tourisme" userName={profile?.full_name ?? undefined} />
      <FormationView
