...
function nextFiscalDeadlines(country: FiscalCountry = 'FR'): Array<{ label: string; date: string; description: string; type: 'urgent' | 'normal' }> {
  ...
  if (country === 'ES') {
    ...
    deadlines.push({
      label: 'IRS Categoria B (annuel)',
      date: irsDate,
      description: 'Modelo 3 + Anexo B via portaldasfinancas.gov.pt',
    })

    // IVA trimestriel PT : 20 du 2e mois suivant la fin du trimestre
    const month = today.getMonth()
    const trimEndMonths = [2, 5, 8, 11] // mar, jun, sep, déc
    const nextTrimEnd = trimEndMonths.find(m => m > month) ?? (trimEndMonths[0] + 12)
    const ivaY = nextTrimEnd >= 12 ? year + 1 : year
    const ivaM = nextTrimEnd >= 12 ? nextTrimEnd - 12 + 2 : nextTrimEnd + 2
    const ivaDate = new Date(ivaY, ivaM, 20)
    deadlines.push({
      label: 'IVA trimestrielle',
      date: ivaDate,
      description: 'Si dépassement du seuil 15 000 € (continental). Portal Finanças.',
    })

    // Taxa Turística : variable selon municipalité (Porto, Lisbonne…), reverse mensuel ou trimestriel
    const taxaDate = new Date(year, today.getMonth() + 1, 15)
    deadlines.push({
      label: 'Taxa Turística municipale',
      date: taxaDate,
      description: 'Reversement à la Câmara Municipal (Porto, Lisboa, Faro…)',
    })
  } else {
    // FR — déclaration BIC annuelle (mai)
...
const s: Record<string, React.CSSProperties> = {
  ...
  netBanner: {
    ...
    // Mesh gradient 2026 : radial halo accent + linéaire success
    background: 'radial-gradient(ellipse 60% 80% at 100% 50%, rgba(255,213,107,0.08), transparent 60%), linear-gradient(135deg, var(--success-bg) 0%, transparent 70%)',
    border: '1px solid var(--success-border)',
    borderRadius: 'var(--r-xl)',
    marginBottom: 'var(--s-5)',
    boxShadow: 'var(--shadow-sm)',
  },
  netBannerCol: { display: 'flex', flexDirection: 'column' as const, alignItems: 'center', textAlign: 'center' as const, gap: 'var(--s-1)', minWidth: 0 },
  netBannerLbl: { fontSize: 'var(--t-xs)', fontWeight: 600, letterSpacing: '0.6px', textTransform: 'uppercase' as const, color: 'var(--text-muted)' },
  netBannerVal: { fontSize: 'var(--t-2xl)', fontWeight: 700, fontFamily: 'var(--font-fraunces), serif', lineHeight: 'var(--lh-tight)', letterSpacing: 'var(--ls-tight)' },
  netBannerSub: { fontSize: 'var(--t-xs)', color: 'var(--text-muted)', lineHeight: 'var(--lh-snug)' },
  netBannerArrow: { fontSize: 'var(--t-2xl)', fontWeight: 700, opacity: 0.5 },

  // 3 colonnes Brut → Commission → Net pour chaque canal
  channelStats: { display: 'grid', gridTemplateColumns: '1fr auto 1fr auto 1fr', gap: 'var(--s-3)', alignItems: 'center' },
  channelStatBox: { display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: '2px', minWidth: 0 },
  channelStatLbl: { fontSize: 'var(--t-xs)', fontWeight: 600, letterSpacing: '0.5px', textTransform: 'uppercase' as const, color: 'var(--text-muted)' },
  channelStatVal: { fontSize: 'var(--t-md)', fontWeight: 600, fontFamily: 'var(--font-fraunces), serif', letterSpacing: 'var(--ls-snug)' },
  channelStatSub: { fontSize: 'var(--t-xs)', color: 'var(--text-muted)' },
  channelStatArrow: { fontSize: 'var(--t-base)', fontWeight: 700, opacity: 0.5 },
  ...
  twoCol:   { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 'var(--s-5)' },
  card:     {
    background: 'var(--card-bg)',
    border: '1px solid var(--border)',
    borderRadius: 'var(--r-xl)',
  ...
  cardTitle:{ fontSize: 'var(--t-lg)', fontWeight: 600, color: 'var(--text)', margin: 0, marginBottom: 'var(--s-1)', letterSpacing: 'var(--ls-snug)', fontFamily: 'var(--font-fraunces), serif' },
  cardSub:  { fontSize: 'var(--t-sm)', color: 'var(--text-muted)', margin: '0 0 var(--s-5) 0', lineHeight: 'var(--lh-snug)' },
