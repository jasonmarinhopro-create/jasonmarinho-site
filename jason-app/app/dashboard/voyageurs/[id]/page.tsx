...
export default async function VoyageurPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [voyageurRes, sejoursRes, authRes, profileDataRes, logementsRes] = await Promise.all([
    supabase
      .from('voyageurs')
      .select('id, prenom, nom, email, telephone, nationalite, source, tags, notes, note_privee, id_type, id_url, id_verifie, bloque, bloque_motif, created_at, checkin_token, checkin_sent_at, checkin_completed_at')
      .eq('id', id)
      .eq('user_id', profile.userId)
      .single(),
    supabase
      .from('sejours')
      .select('id, voyageur_id, logement, date_arrivee, date_depart, montant, contrat_statut, contrat_date_signature, contrat_lien, contrat_plateforme, commission_montant')
      .eq('voyageur_id', id)
      .eq('user_id', profile.userId)
