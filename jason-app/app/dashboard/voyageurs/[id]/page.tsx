...
export default async function VoyageurPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [voyageurRes, sejoursRes, authRes, profileDataRes, logementsRes] = await Promise.all([
    ...
    supabase
      .from('sejours')
      .select('id, voyageur_id, logement, date_arrivee, date_depart, montant, contrat_statut, contrat_date_signature, contrat_lien, contrat_plateforme, commission_montant, annule_at')
      .eq('voyageur_id', id)
      .eq('user_id', profile.userId)
