...
export default function PhotographersAdmin({ active, pendingPayment, hidden, cancelled, founderActiveCount }: Props) {
  ...
  function handleHide(p: Photographer) {
    ...
    startBusy(async () => {
      ...
      else { setOk(`${p.full_name} masqué.`); router.refresh() }
    })
  }
  function handleUnhide(p: Photographer) {
    setErr(null); setOk(null)
    startBusy(async () => {
      const res = await unhidePhotographer(p.id)
      if (res.error) setErr(res.error)
  ...
  function handleDeleteOrphan(p: Photographer) {
    ...
    startBusy(async () => {
      ...
      if (res.error) setErr(res.error)
      else { setOk(`${p.full_name} supprimé.`); router.refresh() }
    })
  }
  ...
  return (
    <section style={s.wrap}>
      ...
      {/* ACTIFS — liste principale */}
      <h3 style={s.sectionTitle}>
        <CheckCircle size={14} weight="fill" /> Fiches actives dans l'annuaire · {active.length}
      </h3>
      {active.length === 0 ? (
          ...
          <div style={s.table}>
            ...
            {pendingPayment.map(p => (
              <div key={p.id} style={s.row}>
                ...
                <div style={s.cellMid}>il y a {fmtAge(p.created_at)}</div>
                <div style={s.actions}>
                  <button onClick={() => handleDeleteOrphan(p)} disabled={busy} style={s.btnDanger}>
          ...
          <div style={s.table}>
            {hidden.map(p => (
              <div key={p.id} style={s.row}>
                ...
                <div style={s.cellMid}>{p.ville}</div>
                <div style={s.cellMid}>{p.email}</div>
                <div style={s.cellMid}>masqué il y a {fmtAge(p.updated_at)}</div>
                <div style={s.actions}>
                  <button onClick={() => handleUnhide(p)} disabled={busy} style={s.btnPrimary}>
