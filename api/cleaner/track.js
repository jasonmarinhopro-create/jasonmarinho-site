...
module.exports = async function handler(req, res) {
  ...
  if (!slug || !['view', 'site', 'instagram'].includes(event)) {
  ...
  const lookupRes = await fetch(
    `${SUPABASE_URL}/rest/v1/cleaners?slug=eq.${encodeURIComponent(slug)}&status=eq.active&is_public=eq.true&select=id&limit=1`,
  ...
  const cleanerId = matches[0].id

  const rpc = event === 'view'
    ? { fn: 'increment_cleaner_views', args: { p_id: cleanerId } }
    : { fn: 'increment_cleaner_click', args: { p_id: cleanerId, p_kind: event } }
  await fetch(`${SUPABASE_URL}/rest/v1/rpc/${rpc.fn}`, {
    method: 'POST',
    headers: { apikey: SERVICE_KEY, Authorization: `Bearer ${SERVICE_KEY}`, 'Content-Type': 'application/json' },
    body: JSON.stringify(rpc.args),
  }).catch(err => console.warn('[cleaner/track] rpc failed', err))
