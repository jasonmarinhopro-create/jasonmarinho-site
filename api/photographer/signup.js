...
module.exports = async function handler(req, res) {
  ...
  const checkUrl = `${SUPABASE_URL}/rest/v1/photographers?email=eq.${encodeURIComponent(email)}&select=id,status,stripe_subscription_id,user_id&limit=1`
  ...
  if (checkRes.ok) {
    ...
    if (Array.isArray(existing) && existing.length > 0) {
      ...
      if (isOrphan) {
        console.warn('[photographer/signup] orphan detected, cleaning up before retry', row.id)
        await rollback({
