...
- **Clics affiliés** (sept. 2026) : table `affiliate_clicks` (migration `20260926_106`, `session_id`, `path`, `url`, `partner` déduit du domaine : `app.lodgify.com` → `lodgify`), écrite par `api/track/click.js` via un écouteur de clic dans `nav.js` sur tout lien `a[rel~="sponsored"]` (donc tout nouveau lien affilié est suivi automatiquement s'il porte bien `rel="sponsored"`). Affichée dans `/dashboard/admin` (`AffiliateClicksCard`, 30 jours : total, par partenaire, pages qui envoient des clics). Un clic n'est pas une vente : les leads/ventes restent dans le tableau de bord affilié de chaque partenaire.
