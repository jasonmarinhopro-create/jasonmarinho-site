...
- ⚠️ **Mention TVA par défaut** : "TVA non applicable, article 261 D 4° du CGI (location meublée de tourisme)" — correspond à la location de meublé de tourisme SANS prestations parahôtelières, le cas le plus courant en LCD. Ce n'est PAS la même chose que la franchise en base (art. 293B) : ne pas confondre si un hôte a un statut différent (para-hôtellerie, hors France, TVA applicable…), il doit adapter la mention lui-même dans son profil.
...
- ⚠️ **Cas Alojamento Local (Portugal)** : vérifié par recherche web (sept. 2026) — au Portugal, la facturation en Alojamento Local est **obligatoire** (même via Airbnb/Booking), **toujours avec IVA à 6%** (5% Madère, 4% Açores, jamais "non applicable"), et **exclusivement** via le Portail des Finances (e-fatura, gratuit) ou un logiciel certifié AT depuis 2024 (facture Excel/Word explicitement interdite). `app/invoice/[token]/page.tsx` affiche donc un bandeau d'avertissement (`isPortugueseAL`, quand `contract.pays === 'PT'`) précisant que ce document n'est PAS une fatura valide et renvoyant vers le Portail des Finances — la page ne prétend jamais être conforme pour un logement portugais, contrairement au cas FR (exonération TVA) où elle l'est.
